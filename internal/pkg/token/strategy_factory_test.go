...
func TestStrategyFactory_Prepend(t *testing.T) {
	t.Run("Prepend", func(t *testing.T) {
		...
		assert.Equal(t, token.Token{
			...
			Raw:       `%getEnv("HOST")%`,
			DependsOn: nil,
			Code:      `func() (r interface{}, err error) { r, err = caller.CallProvider(pkg.GetEnv, "HOST"); if err != nil { err = fmt.Errorf("%s: %w", "cannot execute %getEnv(\"HOST\")%", err) }; return }`,
		}, tk)
	})
	...
	t.Run("RegisterFunc", func(t *testing.T) {
		...
		assert.Equal(t, token.Token{
			...
			Raw:       `%secret("password")%`,
			DependsOn: nil,
			Code:      `func() (r interface{}, err error) { r, err = caller.CallProvider(secrets.Secret, "password"); if err != nil { err = fmt.Errorf("%s: %w", "cannot execute %secret(\"password\")%", err) }; return }`,
		}, tk)
		...
		assert.Equal(t, token.Token{
			...
			Raw:       `%secret("password")%`,
			DependsOn: nil,
			Code:      `func() (r interface{}, err error) { r, err = caller.CallProvider(anotherSecrets.SuperSecret, "password"); if err != nil { err = fmt.Errorf("%s: %w", "cannot execute %secret(\"password\")%", err) }; return }`,
		}, tk)
	})
}
